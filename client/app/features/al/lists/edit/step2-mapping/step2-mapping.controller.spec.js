...
`;
  ...
  describe('#finishMap', () => {
    ...
    it('contacts fields key invalid, return false', () => {
      ...
      expect(MappingComponent.message.show).to.equal(true);
      expect(MappingComponent.message.text).to.equal('Contact Fields "number2 , first_name" are marked as keys but has no mapped source field/index');
      expect(MappingComponent.message.expires).to.equal(8000);
    });
    ...
    it('all multiple mapped fields from a key number must be mapped to Soruce Index', () => {
      ...
      expect(MappingComponent.finishMap()).to.equal(null);
      expect(MappingComponent.message.show).to.equal(true);
      expect(MappingComponent.message.text).to.equal('Contact Fields "number2 , company" are marked as keys but has no mapped source field/index');
      expect(MappingComponent.message.expires).to.equal(8000);
    });
    ...
    it('all multiple mapped fields from a key number must be mapped to Source Field', () => {
      ...
      expect(MappingComponent.finishMap()).to.equal(null);
      expect(MappingComponent.message.show).to.equal(true);
      expect(MappingComponent.message.text).to.equal('Contact Fields "number2 , last_name" are marked as keys but has no mapped source field/index');
      expect(MappingComponent.message.expires).to.equal(8000);
    });
  ...
  describe('Support more formats in phone numbers', function () {
    it('Number Phones with non numeric characters should be cleaned and validated', () => {
      ...
      MappingComponent.contactFields = [
        { 'name': 'number1', mappedName: 'number1', type: 'PHONE', mappedIndex: 0, isKey: true },
        { 'name': 'number2', mappedName: null, mappedIndex: 0, isKey: false },
        { 'name': 'number3', mappedName: null, mappedIndex: 0, isKey: false },
        { 'name': 'last_name', mappedName: null, mappedIndex: 0, isKey: false },
        { 'name': 'company', mappedName: null, mappedIndex: 0, isKey: false }
      ];
      ...
      expect(rows[4]).to.eql({ number1: '5553224432' });
      expect(rows[5]).to.eql({ number1: '3384023456' });
      expect(rows[6]).to.eql({ number1: '7646438893' });
      expect(rows[7]).to.eql({ number1: '0112335532245223' });
      expect(rows[8]).to.eql({ number1: '3323953563' });
    });
  });
  ...
  describe('Support more formats in phone numbers in 2 columns of CSV', function () {
    it('Number Phones with non numeric characters should be cleaned and validated', () => {
      ...
      MappingComponent.contactFields = [
        { 'name': 'number1', mappedName: 'number1', mappedIndex: 0, type: 'PHONE', isKey: true },
        { 'name': 'number2', mappedName: 'number2', mappedIndex: 0, type: 'PHONE', isKey: true }
      ];
      ...
      expect(rows[1]).to.eql({ 'number1': '011663342346742', 'number2': '9632587410' });
      expect(rows[2]).to.eql({ 'number1': '01187859302098742', 'number2': '' });
      expect(rows[3]).to.eql({ 'number1': '5553224432', 'number2': '9788874987' });
      expect(rows[4]).to.eql({ 'number1': '7646438893', 'number2': '01196325874102' });
      expect(rows[5]).to.eql({ 'number1': '3363434124', 'number2': '011963258741024' });

    });

    it('Record is valid if just one the phone numbers are valid', () => {
      ...
      let mockUpdateSettings = {
        ...
        listAddMode: 'ADD_FIRST',
        fieldsMapping: [{ 'fieldName': 'number1', 'key': true, 'columnNumber': 1 },
          { 'fieldName': 'number2', 'key': true, 'columnNumber': 2 }]
      };
      ...
      MappingComponent.contactFields = [
        { 'name': 'number1', mappedName: 'numero1', isKey: true, type: 'PHONE' },
        { 'name': 'number2', mappedName: 'numero2', isKey: true, type: 'PHONE' },
        { 'name': 'number3', mappedName: null, isKey: false, type: 'PHONE' },
      ...
      expect(rows[0]).to.eql({ 'number1': '', 'number2': '01156832', 'first_name': 'Boris', 'company': 'ninguna' });
    });
    ...
    it('When remove numbers are not required', () => {
      ...
      MappingComponent.contactFields = [
        { 'name': 'number1', mappedName: 'numero1', isKey: true, type: 'PHONE' },
        { 'name': 'number2', mappedName: 'numero2', isKey: true, type: 'PHONE' },
        { 'name': 'number3', mappedName: null, isKey: false, type: 'PHONEt' },
        { 'name': 'first_name', mappedName: 'fName', isKey: false },
        { 'name': 'last_name', mappedName: null, isKey: false },
        { 'name': 'company', mappedName: 'Compa', isKey: false }
      ];
